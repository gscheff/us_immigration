...
class I94Ingestion:
    ...
    def __init__(self, source, table, database, bucket, prefix):
        self.source = source
        self.table = table
        self.database = database
        self.target = (bucket / prefix / table).url
        self.partition_cols = ['year', 'month', 'day']

    def read(self):
        log.info('read dataframe from %s' % self.source)
        return pd.read_parquet(self.source)

    def write(self, df):
        log.info('write dataframe to %s' % self.target)
        options = dict(
            dataset=True,
    ...
    def process(self, df):
        ...
        for col, dtype in zip(df.columns, df.dtypes):
            if dtype.name == 'float64':
                df[col] = df[col].astype('Int64')  # use nullable integer

        df['datetime'] = (
            pd.to_timedelta(df.arrdate, unit='D') + pd.Timestamp('1960-01-01')
        )
        ...
        # check columns
        expected_cols = list(self.columns_comments.keys())
    ...
    def __call__(self):
        df = self.read()
        df = self.process(df)
        result = self.write(df)
        return result


class DemographicsIngestion:
    ...
    def read(self):
        log.info('read dataframe from %s' % self.source)
        return pd.read_csv(self.source, sep=';', encoding='utf8')
...
class CountryIngestion:
    ...
    def read(self):
        log.info('read dataframe from %s' % self.source)
        return pd.read_csv(self.source, encoding='utf8')
...
class RegionIngestion:
    ...
    def __init__(self, source, table, database, bucket, prefix):
        ...
        self.database = database
        self.target = (bucket / prefix / table).url
        self.partition_cols = ['iso_country']
...
class I94visaIngestion:

    def __init__(self, table, database, bucket, prefix):
        self.table = table
        self.database = database
        self.target = (bucket / prefix / table).url
        self.partition_cols = None
...
class I94modeIngestion:
    ...
    def read(self):
        ...
        df = pd.read_csv(io.StringIO(i94mode), sep='=')
...
class I94cntyIngestion:
    ...
    def read(self):
        ...
        df = pd.read_csv(io.StringIO(i94cnty), sep='=')
        df.columns = ['code', 'name']
        for col in df.columns[1:]:
            df[col] = df[col].str.strip(to_strip=" '\t")
        df['name'] = df['name'].str.title()
